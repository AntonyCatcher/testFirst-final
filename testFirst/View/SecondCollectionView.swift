...
extension SecondCollectionView: UICollectionViewDelegate, UICollectionViewDataSource, UICollectionViewDelegateFlowLayout {
    ...
    // Method to determine when the last item is about to be displayed
    func collectionView(_ collectionView: UICollectionView, willDisplay cell: UICollectionViewCell, forItemAt indexPath: IndexPath) {
        // Load the next batch of data when the last item is about to be displayed
        if indexPath.item == labelData.count - 1 {
            loadMoreData()
        }
    }

    // Method to load the next batch of data
    func loadMoreData() {
        // Add the newly loaded data to the existing data source
        let newData = ["iOS", "Android", "Design", "Flutter", "QA", "PM", "Pyton", "Kotlin", "Java", "React"] // load the next batch of data
        labelData.append(contentsOf: newData)
        reloadData()
    }
